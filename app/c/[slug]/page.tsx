...
export async function generateMetadata({
  ...
  const query = supabase
    .from("posts")
    .select(
      `title, description, image, categories(name), seo_title, seo_description, seo_keywords, published_at, author, slug`,
    )
    .eq("isdeleted", false);
  ...
  const canonicalSlug =
    p.slug || (isNumericId ? String(resolvedParams.slug) : resolvedParams.slug);
  ...
  const derivedKeywords = extractKeywordsBG(
  ...
  const finalKeywords = Array.from(
    new Set([
      ...(Array.isArray(p.seo_keywords) ? p.seo_keywords : []),
      ...derivedKeywords,
    ]),
  );
  ...
  const description =
    p.seo_description ||
    p.description ||
    `${p.title} - ${metaCat?.name || "Stox.bg"}`;
  const ogImage =
    p.image ||
    `/api/og?title=${encodeURIComponent(p.title)}&subtitle=${encodeURIComponent(metaCat?.name || "Анализ")}&type=analysis`;
  const publishedTime = p.published_at
...
function processContent(content: string): string {
  ...
  processed = processed.replace(
    /^https?:\/\/[^\s]+\.(jpg|jpeg|png|gif|webp)(\?[^\s]*)?$/gm,
    "",
  );
...
function FormattedContent({ content }: { content: string }) {
  ...
  const components = {
    ...
    h1: ({
      ...
      <h1 {...props} className="text-3xl font-bold my-8">
        {children}
      </h1>
    ),
    h2: ({
      ...
      <h2 {...props} className="text-2xl font-bold my-6">
        {children}
      </h2>
    ),
    h3: ({
    ...
    p: ({
      ...
      <p {...props} className="my-4 text-base leading-relaxed">
        {children}
      </p>
    ),
...
export default async function PostPage({
  ...
  const categoryObj:
    | { id?: number; name?: string; color?: string }
    | undefined = Array.isArray(joinedCategory)
    ? joinedCategory[0]
    : joinedCategory;
  ...
  const publishedISO = (post as PostRecord).published_at
    ? new Date((post as PostRecord).published_at as string).toISOString()
    : new Date().toISOString();
  const articleSchema = generateArticleSchema({
    title:
      (post as { seo_title?: string; title: string }).seo_title ||
    ...
    description:
      ...
      (post as { description?: string; title: string }).description ||
      (post as { title: string }).title,
        ...
        <header className="mb-10">
          {categoryObj && (
            <span
              ...
              style={{
                backgroundColor: categoryObj?.color || "#e5e5e5",
                color: "#fff",
              }}
            >
          ...
          <div className="flex flex-wrap justify-between items-center gap-4 border-t border-b border-border py-4">
            <div className="flex items-center gap-4">
              <div className="flex items-center text-sm text-muted-foreground">
                ...
                <span>
                  {new Date(publishedISO).toLocaleDateString("bg-BG", {
                    year: "numeric",
                    month: "long",
                    day: "numeric",
                  })}
